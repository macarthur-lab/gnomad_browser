...
CMD echo "Creating directories..." && \
  ...
  gsutil -m cp /var/data/tabixtemp/exomes/*tbi gs://gnomad-browser/exomes/feb-2017-test/ && \
  gsutil -m cp /var/data/tabixtemp/genomes/*tbi gs://gnomad-browser/genomes/feb-2017-test/ && \
  echo "Cleaning up" && \
